...
class ShopsTable extends Table
{
    ...
    public function verifyShopAccount($account, $password)
    {
        $hasher = new DefaultPasswordHasher();
        $query = $this->find()->select(['id', 'password'])->where(['account' => $account])->first();
        
        if(is_null($query)) return false;

        $check = $hasher->check($password, $query->password);

        if ($check) return $query->id;
        return $check;
    }
}
